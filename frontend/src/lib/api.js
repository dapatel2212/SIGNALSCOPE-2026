...
// Local mock history store helper
function getLocalMockHistory() {
  try {
    const raw = localStorage.getItem('signalscope_history_items');
    if (!raw) {
      localStorage.setItem('signalscope_history_items', JSON.stringify(INITIAL_MOCK_HISTORY));
...
function appendLocalMockHistory(detail) {
  try {
    const list = getLocalMockHistory();
    const newItem = {
      id: detail.id,
      label: detail.label,
...
export async function checkBackendStatus() {
  ...
  try {
    await axios.get(`${BASE_URL}/api/docs/schema/`, { timeout: 3000 });
...
export async function fetchScanHistory(page = 1) {
  ...
  try {
    const response = await apiClient.get(`/api/history/?page=${page}`);
    ...
    const local = getLocalMockHistory();
    return {
      count: local.length,
      next: null,
      previous: null,
...
export async function fetchScanDetail(id) {
  if (isMockModeEnabled()) {
    ...
    const history = getLocalMockHistory();
